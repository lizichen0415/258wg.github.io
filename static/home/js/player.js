if (!/(Android|iPhone|iPod|Ipad|ios)/i.test(navigator.userAgent)) {
	...
	var audio = new Audio(),
		...
		roundcolor = "#6c6971",
		lightcolor = "#81c300",
		playListFile = "plst.js"/*tpa=http://www.httpswww.144fz.com/static/home/js/plst.js*/,
		albumListId = 0,
		volume = 0.50, // 初始音量
		lrcshow = true,
		songListId = 0,
	...
	function wenkmCicle() {
		...
		if (audio.currentTime < audio.duration / 2) {
			$btns.css("background-image", "linear-gradient(90deg, " + roundcolor +
				" 50%, transparent 50%, transparent), linear-gradient(" + (90 + (270 - 90) / (audio.duration / 2) * audio.currentTime) +
				"deg, " + lightcolor + " 50%, " + roundcolor + " 50%, " + roundcolor + ")")
		} else {
			$btns.css("background-image", "linear-gradient(" + (90 + (270 - 90) / (audio.duration / 2) * audio.currentTime) +
				"deg, " + lightcolor + " 50%, transparent 50%, transparent), linear-gradient(270deg, " + lightcolor + " 50%, " +
	...
	var wenkmMedia = {
		...
		getInfos: function(id) {
			songListId = id;
			$.ajax({
				url: "http://www.xiami.com/song/playlist/id/" + wenkmList[albumListId].song_list[songListId].song_id +
					"/cat/json?callback=?",
				dataType: "json",
				success: function(infos) {
					...
					$(".list li", $player).eq(songListId).addClass("current").find(".artist").html("&nbsp;-&nbsp;" + infos.data.trackList[
						0].artist).parent().siblings().removeClass("current");
					audio.volume = volume;
					audio.play();
					wenkmLrc.load()
				},
...
$(document).ready(function() {
	$(window).keydown(function(a) {
		a = a.keyCode;
		192 == a ? audio.paused ? (audio.play(), $(".list li", $player).eq(songId).addClass("current").find(".artist").html(
			"正在播放&nbsp;>&nbsp;").parent().siblings().removeClass("current")) : (audio.pause(), $(".list li", $player).eq(
			songId).addClass("current").find(".artist").html("暂停播放&nbsp;>&nbsp;").parent().siblings().removeClass(
			"current")) : 37 == a ? (wenkmMedia.prev(), $(".myhk_pjax_loading_frame,.myhk_pjax_loading").css("display",
			"block")) : 39 == a && (wenkmMedia.next(), $(".myhk_pjax_loading_frame,.myhk_pjax_loading").css("display",
			"block"))
	})
});
